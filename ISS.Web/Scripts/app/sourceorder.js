...
requisitions = {
    ...
    readonlyFieldsReq: function (flag) {
        ...
        $("#Mode").data("kendoDropDownList").readonly(flag);
    ...
    clearAllFieldsNewReq: function () {
        ...
        // $('#btnRetOrderClear').click() //CA#56079-18- Travel to create requisition
    ...
    DeleteRequisition: function () {
        if ($("#frmRequisitions #RequisitionId").val().trim() != '') {
            if ($('#frmRequisitions #FormMode').val() == "Edit") {
                var postData = { RequisitionId: $("#frmRequisitions #RequisitionId").val() };
                ISS.common.showConfirmMessage('Are you sure to delete this requisition ?', null, function (stat) {
                    if (stat) {
                        postData = JSON.stringify(postData);
                        ISS.common.executeActionAsynchronous("../order/DeleteRequisition", postData, function (stat, data) {
                            if (stat) {
                                if (data.Status) {
                                    requisitions.clearAllFieldsNewReq();
                                ...
                                ISS.common.showPopUpMessage('Failed to delete the Requisition.');
                            }
                        });
                    }
                });
                ...
                ISS.common.showPopUpMessage("ADD mode. Cannot delete. Must clear first.");
                ISS.common.notify.error("ADD mode. Cannot delete. Must clear first.");
            }
        }
        else {
    ...
    checkFutureDate: function () {
        ...
        if (requisitions.isValidFormMode()) {
            ...
            tt.setDate(365)
            var EnteredDate = new Date($("#PlannedDcDate").val());
            if (EnteredDate == 'Invalid Date') {
                ...
                postData = JSON.stringify(postData);
                ISS.common.executeActionAsynchronous("../order/GetDcValidates", postData, function (stat, data) {
                    if (stat) {
                        if (data.length == 0) {
        ...
        document.addEventListener('focusout', function (e) {
            ...
            if (event.target.id == 'Style') {
                ...
                if (grid) {
                    ...
                    postData = JSON.stringify(postData);
                    ISS.common.executeActionAsynchronous("../order/GetRetrieve", postData, function (stat, data) {
                        if (stat) {
