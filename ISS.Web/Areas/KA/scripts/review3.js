...
temp = {
    ...
    gridDataBoundStyle: function (e) {
        ...
        for (var i = 0; i < gridData.length; i++) {
            ...
            row.find(".retOrdQty").text(getQtyToEachDisp(gridData[i].Qty));
        }
    ...
    InsertRequisition: function () {
        if (requisitions.isValidFormMode()) {
            if (requisitions.const.validator.validate()) {
                ...
                var url = 'UpdateBulkOrder';
                
                var PostData = {
                    req: ISS.common.getFormData($('#frmRequisitions')),
                   
                };
                if (PostData.req.ProgramSource != requisitions.const.ProgramSrc.AVYX)
                    PostData.reqDet = dsData;

                PostData.req.ReqDetailTracking = $('#ReqDetailTracking').prop('checked'); 
                PostData.req.DetailTrkgInd = $('#DetailTrkgInd').prop('checked');
              

                ISS.common.executeActionAsynchronous(url,JSON.stringify( PostData), function (stat, data) {
                    ISS.common.blockUI();
                    if (stat && data) {
                        if (data.Key) {
                        ...
                        ISS.common.showPopUpMessage('Failed to update bulk order details.');
                    }
            ...
            ISS.common.notify.error('Please enter bulk order number.');
        }
    },
    ...
    validateRequisitionDetail: function (callback) {
        ...
        ISS.common.executeActionAsynchronous(requisitions.const.urlvalidateRequisitionDetail, postData, function (stat, data) {
            if (stat) {
                if (data) {
    ...
    validateRequisitionDetailRow: function (row, callback) {
        ...
        postData = JSON.stringify(PostData);
        ISS.common.executeActionAsynchronous(requisitions.const.urlvalidateRequisitionDetailRow, JSON.stringify(PostData), function (stat, data) {
            if (stat) {
                if (data) {
                    if (data.ErrorStatus) {
    ...
    removeOrderDetail: function (e) {
        if (requisitions.isValidFormMode()) {
            ...
            var select = dataItem.ProcessedToOS;
            if (select == "Y" || select == "I") {
                $('#grdRequisitionDetail').data("kendoGrid").closeCell();
                var msg = "Unable to Delete the row as Processed To OS Status is " + select;
                ISS.common.notify.error(msg);
                return false;
            }
            if (dataItem) {
    ...
    isValidFormMode: function () {
        return ($("#frmRequisitions #BulkNumber").val() != "" && $('#frmRequisitions #FormMode').val() == "Edit")        
    },
    ...
    onStyleCdClick: function (e) {
        ISS.common.executeActionAsynchronous(requisitions.const.urlGetRetrieve, null, loadDescriptionAndColor);
    },
    ...
    GetDuplicateSingle: function () {
        ...
        ISS.common.executeActionAsynchronous(requisitions.const.urlGetDuplicateRecordsBulkSingle, JSON.stringify(postData), function (stat, data) {
            if (stat && data) {
                ...
                var i = 0;
                GetLineNumberBulk(dt, data.newList.length, function (Linelist) {
                    if (Linelist) {
                        $(data.newList).each(function (idx, item) {
                            requisitions.changeDetDates(item);
                            item.Cloned = {};
                            item.LineNumber = Linelist[i];
                            dt.push(item);
                            i++;
                        });
                    }
        ...
        if (requisitions.isValidFormMode()) {
            ...
            if (checkedItems.length == 1) {
                ...
                if (grid) {
                    ...
                    ISS.common.executeActionAsynchronous(requisitions.const.urlGetSkuSize, JSON.stringify(postData), function (stat, data) {
                        if (stat && data) {
                ...
                ISS.common.showConfirmMessage('You have selected multiple items. Are you sure want to duplicate same records?', null, function (ret) {
                    if (ret) {
                        ...
                        var postData = { list: arr }
                        ISS.common.executeActionAsynchronous(requisitions.const.urlGetDuplicateRecords, JSON.stringify(postData), function (stat, data) {
                            if (stat && data) {
                                var dt = gridDetail.dataSource.data();
                                var i = 0;
                                GetLineNumberBulk(dt, data.length, function (Linelist) {
                                    if (Linelist) {
                                        $(data).each(function (idx, item) {
                                            requisitions.changeDetDates(item);
                                            item.Cloned = {};
